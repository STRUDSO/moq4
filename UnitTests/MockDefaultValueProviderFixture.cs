...
namespace Moq.Tests
{
	public class MockDefaultValueProviderFixture
	{
		...
		public void ProvidesMockValue()
		{
            ...
            var value = mock.ProvideDefault(typeof(IFoo).GetProperty("Bar").GetGetMethod());
		...
		public void CachesProvidedValue()
		{
		    ...
		    var value1 = mock.ProvideDefault(typeof(IFoo).GetProperty("Bar").GetGetMethod());
            var value2 = mock.ProvideDefault(typeof(IFoo).GetProperty("Bar").GetGetMethod());
		...
		public void ProvidesEmptyValueIfNotMockeable()
		{
            ...
            var value = mock.ProvideDefault(typeof(IFoo).GetProperty("Value").GetGetMethod());
			Assert.Equal(default(string), value);

            value = mock.ProvideDefault(typeof(IFoo).GetProperty("Value").GetGetMethod());
			Assert.Equal(default(string), value);

            value = mock.ProvideDefault(typeof(IFoo).GetProperty("Indexes").GetGetMethod());
			Assert.True(value is IEnumerable<int> && ((IEnumerable<int>)value).Count() == 0);

            value = mock.ProvideDefault(typeof(IFoo).GetProperty("Bars").GetGetMethod());
